...
namespace TPLabo2_Ajedrez
{
    class Caballo
    {
        ...
        public bool PosicionarCaballos(int [,] MatrizPrueba , sPieza [] LOOKUP, sPieza CABALLO)
        {
        ...
        public void RecursividadCaballo(int [,] MatrizPrueba, sPieza[] LOOKUP, sPieza CABALLO, int i, int j)
        {
            int NLibres = 0;
            if(i >= 0 && i < N && j >= 0 && j < N)
            {
        ...
        public int VerificarPosCaballo(int [,] MatrizPrueba, sPieza CABALLO)
        {
            cont = 8;
            if(CABALLO.posicion.FILA + 1 < N)
            {
                if (CABALLO.posicion.COL - 2 > 1) cont -= MatrizPrueba[CABALLO.posicion.FILA + 1, CABALLO.posicion.COL - 2];
                if (CABALLO.posicion.COL + 2 < N) cont -= MatrizPrueba[CABALLO.posicion.FILA + 1, CABALLO.posicion.COL + 2];
                if(CABALLO.posicion.FILA + 2 < N)
                {
                    if (CABALLO.posicion.COL - 1 > 1) cont -= MatrizPrueba[CABALLO.posicion.FILA + 2, CABALLO.posicion.COL - 1];
                    if (CABALLO.posicion.COL + 1 < N) cont -= MatrizPrueba[CABALLO.posicion.FILA + 2, CABALLO.posicion.COL + 1];
                }
            }
